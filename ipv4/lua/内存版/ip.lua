...
function ipdata.LoadFile(path)
    ...
    ipdata.data = redaFileBytes(ipdata.file, 257 * 8 + 4, 0)
    for k = 1, 256 do
        local i = k * 8 + 4
        ipdata.prefStart[k] = unpackInt4byte(ipdata.data[i + 1], ipdata.data[i + 2], ipdata.data[i + 3], ipdata.data[i + 4])
        ipdata.prefEnd[k] = unpackInt4byte(ipdata.data[i + 5], ipdata.data[i + 6], ipdata.data[i + 7], ipdata.data[i + 8])
    end

    local recordSize = unpackInt4byte(ipdata.data[1], ipdata.data[2], ipdata.data[3], ipdata.data[4])

    local allsizeData = redaFileBytes(ipdata.file, recordSize*9+9, 2052)
    for i = 0, (recordSize-1) do
        local x = i * 9
		local a = 1 + x
		local b = 2 + x
		local c = 3 + x
		local d = 4 + x
        local offset = unpackInt4byte(allsizeData[d + 1], allsizeData[d+2], allsizeData[d+3], allsizeData[d+4])
        local len = allsizeData[d+5] % 256
        ...
        local tempData = ipdata.file:read(len)
        ipdata.addrArr[i] = tempData
    end
end
