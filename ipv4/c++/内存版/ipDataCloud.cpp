...
int32_t ip_loadDat(geo_ip* p)
{
	...
	errno_t err = fopen_s(&file, "D:\\files\\databases\\ip4_proxy.dat", "rb");
	if (err == 2)
	{
	...
	{
		i = k * 8 + 4;
		p->prefStart[k] = ip_read_int32(p, buffer, i);
		p->prefEnd[k] = ip_read_int32(p, buffer, i + 4);
	}

	RecordSize = ip_read_int32(p, buffer, 0);
	p->endArr = (uint32_t*)malloc(RecordSize * sizeof(uint32_t));
	p->addrArr = (char**)malloc(RecordSize * sizeof(char*));
	for (i = 0; i < RecordSize; i++)
	{
		j = 2052 + (i * 9);
		p->endArr[i] = ip_read_int32(p, buffer, j);
		offset = ip_read_int24(p, buffer, 4 + j);
		length = (uint32_t)buffer[8 + j];
