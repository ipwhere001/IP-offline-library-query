...
#include "uint128_t.h"
 ...
 char IPV6_FILENAME[] = "your file path";
...
char* get_addr(geo_ipv6* p, uint32_t cur) {
	...
	offset = ipv6_read_int32(p, p->data, j + 50);
...
int32_t ipv6_loadDat(geo_ipv6 *p)
{
	...
	p->numbers = ipv6_read_int32(p,p->data,4);

	for (k = 0; k < p->numbers; k++)
	{
		i = k * 12 + 4 + 4;
		p->prefStart[ipv6_read_int32(p, p->data, i+8)] = ipv6_read_int32(p, p->data, i);
		p->prefEnd[ipv6_read_int32(p, p->data, i + 8)] = ipv6_read_int32(p, p->data, i + 4);
	}

	RecordSize = ipv6_read_int32(p, p->data, 0);
	p->endArr = (char **)malloc(RecordSize * sizeof(char*));
	for (i = 0; i < RecordSize; i++)
	{
		j = p->numbers *12 + 4 + 4 + (i * 55);
...
long ipv6_binary_search(geo_ipv6 *p, long low, long high, uint128_t k)
{
	...
	while (low <= high)
	{
		...
		uint128_t endipNum = uint128_t(p->endArr[mid],10);
		if (endipNum >= k)
		{
			M = mid;
			if (mid == 0)
