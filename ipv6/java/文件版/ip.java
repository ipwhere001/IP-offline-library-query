...
public class Ip {
    ...
    private Ip(String filepath) {
        ...
        numbers = unpackInt4byte(data[4], data[5], data[6], data[7]);
...
//            long endipnum = unpackInt4byte(data[p+50], data[1 + p+50], data[2 + p+50], data[3 + p+50]);
    ...
    public static synchronized Ip getInstance(String filepath) {
        if (instance == null)
            instance = new Ip(filepath);
        return instance;
    }
